...
def ANSDefects(ang,z,T,h=8.5,od=3.5):
    ...
    return CircumferentialLength[1]-CircumferentialLength[0],Depth,OffCentreLinePosition,CircumferentialLength
...
def TransmissionCoeffs(th):

    from numpy import sin,cos

    sqrt = lambda x: np.sqrt(x+0j)

    C = np.array([[-1.0*1j*sqrt(-sin(th)**2/cw**2 + csL**(-2)), 1.0*1j*sin(th)/cw, -1.0*1j*cos(th)/cw], [2.0*csT**2*rhos*sin(th)**2/cw**2 - 1.0*rhos, 2.0*csT**2*rhos*sqrt(-sin(th)**2/cw**2 + csT**(-2))*sin(th)/cw, -1.0*rhow*cos(th)**2], [-2.0*csT**2*rhos*sqrt(-sin(th)**2/cw**2 + csL**(-2))*sin(th)/cw, 2.0*csT**2*rhos*sin(th)**2/cw**2 - 1.0*rhos, 0]])
    b = np.array([-1.0*1j*cos(th)/cw, 1.0*rhow*cos(th)**2, 0]).reshape(-1,1)
...
# pth = '/Volumes/STORAGE/DestructiveResults/'
#
#
# d = os.listdir(pth)
#
...
# #     k = d[i].strip('.p')
# #     #
# #     # f,ax = subplots(nrows = 2,sharex=True)
# #     #
# #     # f.suptitle(k)
# #
# #
...
# #     #
# #     #
# #     f,ax = subplots(nrows = 4,sharex=True)
# #
# #     f.suptitle(k)
# #
# #     ax[0].plot(dl[k][5:-5],marker='o')
# #
# #     ax[0].set_title('Dis-bond Line (mm)')
# #
# #     ax[1].plot(SWn[i]>=th,marker='o')
# #
...
#     tfm1 = sum(tfm[1],axis=1)
#
#     Xtfm += [[tfm0[i],tfm1[i]] for i in range(len(tfm0))]
#
#
#
#
...
#
#
# savefig(pth+'TFMScatterDestructiveResults1p5mm.png',dpi=250)
#
# close('all')
#
# pc = []
...
#     fig.suptitle(dd[0:3])
#
#     ax[0].plot(dl[dd[0:3]],marker='o')
#
#     ax[0].plot(1.5*ones(L))
#
#     ax[0].set_title('Disbond Line + Threshold')
#
#     ax[1].plot(((dl[dd[0:3]])>1.5).astype(int),marker='o')
#
...
#
#     plot(gm.predict(X),marker='o',color='b')
#
#     title(dd[0:3])
#
#     ylim((-0.1,1.1))
#
#     pc.append([sum(dl[dd[0:3]]>1.5)/L, sum(gm.predict(X))/L])
#
#
#
#
...
#     xlabel('Axial Position (mm)')
#
#     savefig(pth+dd[0:3]+'TFMBinary1p5mm.png',dpi=250,box_inches='tight')
#
#     close('all')
#
#
# pc = 100.*array(pc)
#
# pe = 100.*(pc[:,1]-pc[:,0])/pc[:,0]
#
# pickle.dump({'PercentCalls':pc,'Error':pe},open(pth+'Calls1p5mm.p','wb'))
#
#
