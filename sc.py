...
def VelocityAttenuation(s,dt,d,fbnd,df=0.1,db=-20,c0=2.9):
    ...
    # *exp(-1j*2*pi*f*(iy-ix)*dt)

    H = -(Y/X)*exp(-1j*2*pi*f*(iy-ix)*dt)

    phi = unwrap(angle(H))

    H = H[ff]

    f0 = 0.5*(fbnd[1]-fbnd[0])
    ...
    # plot(f,(2*d/((iy-ix)*dt))*ones((1,len(f))))
...
def AmpDelay(x,dt,N=4,asteel=0.042,csteel=6.):
    ...
    for xx in x:
        ...
        try:
            ...
            F.append([A[1],phi[1],A[2],phi[2],A[3],phi[3]])
...
def PrimerGain(K,f=linspace(1.,15.,100),alpha=0.1,d=0.15,c=(1.97,2.8,5.9),rho=(0.94,1.48,7.8)):
    ...
    R12 = (z2-z1+2j*pi*f*z1*z2/K)/(z2+z1+2j*pi*f*z1*z2/K)
    R21 = (z1-z2+2j*pi*f*z1*z2/K)/(z2+z1+2j*pi*f*z1*z2/K)

    TT12 = 4*z1*z2/((z1+z2+2j*pi*f*z1*z2/K)*(z1+z2+2j*pi*f*z1*z2/K))

    R23 = (z3-z2)/(z3+z2)

    H = (R12 + R23*(TT12 - R21*R12)*exp(-2j*pi*f*T)*exp(-c[1]*alpha*T*f))/(1-R23*R21*exp(-2j*pi*f*T)*exp(-c[1]*alpha*T*f))
    G = log(abs(H))
...
def AdhesivePrimerFit(T1,T2,a1,a2,p):
...
def AdhesivePrimerFeatures(x,dt,df=0.05,frng=(3.,10.),d=(0.075,0.6),c=(1.97,2.9),exptype='immersion'):
    ...
    if exptype=='immersion':
        ...
        gates=[(0.5,1.5,0.3,1.),(1.75,4.25,3.,0.01)]

    elif exptype=='contact':

        gates=[(0.,1.,0.3,1.),(1.7,4.,3.,0.01)]
    ...
    for xx in x:
        ...
        x1 = xc[il1:ir1]
        x1 = (x1-mean(x1))*tukey(len(x1),gates[0][3])
        x2 = xc[il2:ir2]
        x2 = (x2-mean(x2))*tukey(len(x2),gates[1][3])
        ...
        dphi = savgol_filter(unwrap(angle(H)),5,2,deriv=1)


        imin = localmin(dphi)
        im = imin[dphi[imin].argmin()]

        # if len(imax)>len(imin):

        #     fmax = f[dphi[imax].argmax()]
        #     fmin = f[dphi[imin].argmin()]



        # T2 = [1/(2*f[im]),1/f[im],3/(2*f[im]),2/f[im],5/(2*f[im])]

        T2 = [1/(2*f[im]),1/f[im],3/(2*f[im]),2/f[im]] #,5/(2*f[im])]


        param = (f.reshape((len(f),1)),H.reshape((len(H),1)))

        a1c1 = abs(rfft(diff(log(abs(H)))/diff(f)))/(len(H)-1)
        a1c1 = a1c1[0]

        # p=polyfit(f,-log(abs(H)),1)

        # a1c1=p[0]

        # print(a1c1/(c[0]*(T-array(T2))))

        plot(abs(H*exp(a1c1*f)))

        show()

        R = [ minimize(AdhesivePrimerFitResidual,[T-TT,TT,a1c1/(T-TT),0.1*c[1]],args=param,bounds=[(1.,3.),(2*d[0]/c[1],2*d[1]/c[1]),(0.04*c[0],0.12*c[0]),(0.07*c[1],0.35*c[1])],method='SLSQP') for TT in T2]

        R = R[array([RR.fun for RR in R]).argmin()]
        ...
        v,r = AdhesivePrimerFit(T1,T2,a1,a2,param)

        s = linspace(0.,1/(2*dt),NFFT/2+1)

        Hm = exp(-2j*pi*s*T1)*exp(-a1*T1*s)*(v[0,0]+v[1,0]*exp(-2j*pi*T2*s)*exp(-a2*T2*s))/(1+v[2,0]*exp(-a2*T2*s)*exp(-2j*pi*T2*s))


        X2m = X1*Hm


        F.append([f,T1,T2,a1/c[0],a2/c[1],v,H,Hm[ff],X2,X2m,irfft(X1),irfft(X2),ifft(hstack((2*X2m,zeros((len(X2m)-1))))),R])
...
def PrimerFit(T,a,p):
    ...
    A = hstack((exp(-a*T*f)*exp(-2j*pi*T*f),ones((len(f),1))))
...
def PrimerFitResidual(x,*params):
    ...
    v,r = PrimerFit(x[0],x[1],params)

    return r

    
def PrimerFeatures(x,dt,gates=None,df=0.05,frng=(2.,10.),d=(0.075,0.6),c=(2.9,5.9)):
    ...
    for xx in x:
        ...
        im = argmin(savgol_filter(unwrap(angle(H)),7,3,deriv=1))
        ...
        R = [ minimize(AdhesivePrimerFitResidual,[T-TT,TT,0.1*c[0],0.1*c[1]],args=param,bounds=[(1.,3.),(2*d[0]/c[1],2*d[1]/c[1]),(0.08*c[0],0.11*c[0]),(0.08*c[1],0.11*c[1])],method='SLSQP') for TT in T2]
        ...
        F.append([f,T1,T2,a1/c[0],a2/c[1],v,H,Hm[ff],X2,X2m,ifft(ifftshift(X2)),ifft(ifftshift(X2m)),R])
...
class Pipe:
    ...
    def ManualScan(self, Locations, samplingFrequency=50, Averages=64):
...
#        from numpy.linalg import norm
        ...
        for l in Locations:
            ...
            input('Press any key to collect signal at location'+str(l))
...
class PipeSet:
    ...
    def ExtractFeatures(self,ScansPerPipe):
        ...
        for p in self.Pipes:
            ...
            p.Features = AdhesivePrimerFeatures(signals,p.SamplingPeriod)
