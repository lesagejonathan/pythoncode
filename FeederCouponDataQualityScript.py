...
for dd in d:
    ...
    F = FMC.LinearCapture(25., f['AScans'], 0.6, 16)

    F.ProcessScans(100,bp=10)

    F.KeepElements(range(16))

    # F.AScans = [F.AScans[i][::-1,::-1,:] for i in range(len(F.AScans))]

    I0 = np.array([np.abs(detrend(F.PlaneWaveSweep(i, np.array([-36.1]), 2.33))) for i in range(len(f['AScans']))])
    ...
    #
    #
    # I1 = np.array([np.abs(detrend(F.PlaneWaveSweep(i, np.array([-36.]), 2.33))) for i in range(len(f['AScans']))])
    #
    # I1 = I1[:,0,:]
    #
    # I1 = I1.transpose()
